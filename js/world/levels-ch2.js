// levels-ch2.js - os setores do Capitulo 2, "GENTILEZA".
//
...
//   9 enfermaria ......... primeiro rele: MAO.
//  10 sala de evidencias . segundo rele: OLHO.
//  11 comunicacoes ....... terceiro rele: VOZ.
//  12 seguranca .......... circuito de 02h14 e a CHAVE.
...
function itensSoltos(lvl, defs) {
  lvl.pego = {};
  // Exposto para sistemas que sorteiam a posição dos suprimentos. O desenho
  // continua pertencendo à fase, mas agora pode ser desligado sem deixar um
  // item fantasma pintado no chão.
...
// Rele de latao do quadro de seguranca. Cada um leva um pictograma gravado
// para o jogador reconhecer no painel sem depender de texto miudo.
function drawRelay(ctx, x, y, mark) {
  ctx.fillStyle = '#332819'; ctx.fillRect(x - 2, y - 2, 18, 16);
  ctx.fillStyle = '#b08f3f'; ctx.fillRect(x, y, 14, 12);
  ctx.fillStyle = '#d3b75f'; ctx.fillRect(x + 1, y + 1, 12, 2);
  ctx.fillStyle = '#665127'; ctx.fillRect(x + 3, y + 4, 8, 6);
  ctx.fillStyle = '#e3cf86';
  if (mark === 'hand') {
    ctx.fillRect(x + 5, y + 5, 4, 4);
    for (let i = 0; i < 4; i++) ctx.fillRect(x + 4 + i * 2, y + 3 + (i & 1), 1, 3);
  } else if (mark === 'eye') {
    ctx.fillRect(x + 3, y + 6, 2, 2); ctx.fillRect(x + 5, y + 5, 4, 4);
    ctx.fillRect(x + 9, y + 6, 2, 2); ctx.fillStyle = '#332819'; ctx.fillRect(x + 7, y + 6, 1, 2);
  } else {
    ctx.fillRect(x + 3, y + 5, 3, 2); ctx.fillRect(x + 5, y + 7, 3, 2);
    ctx.fillRect(x + 8, y + 6, 2, 2); ctx.fillRect(x + 10, y + 4, 1, 2);
  }
  ctx.fillStyle = '#7f858e'; ctx.fillRect(x - 1, y + 3, 2, 6); ctx.fillRect(x + 13, y + 3, 2, 6);
}
...
export function buildCorridor() {
  ...
  // Atalho do posto de seguranca. Primeiro ele ve a porta sem energia;
  // depois de resolver o quadro, ela vira o retorno curto para o corredor.
  const dSec = M.doorFrame(g, 770, GY, 5166);
  inter.push({
    x: dSec.x, y: dSec.y, w: dSec.w, h: dSec.h, prompt: 'prompt_open',
    action: 'goto', to: 'ch2_security', tox: 650, tofacing: -1, range: 30,
    isDoor: true, precisa: 'puzzle_solved', semChave: 'b2_security_locked',
  });
  rect(g, 764, GY, 38, 3, '#2b2620');
...
export function buildLockerRoom() {
  ...
  // A antiga enfermaria fica espremida entre o vestiario e o banheiro.
  // Esta porta inicia um dos dois ramos da busca pelos reles.
  const dInf = M.doorFrame(g, 500, GY, 5666);
  inter.push({
    x: dInf.x, y: dInf.y, w: dInf.w, h: dInf.h, prompt: 'prompt_open',
    action: 'goto', to: 'ch2_infirmary', tox: 72, tofacing: 1, range: 30, isDoor: true,
  });
  rect(g, 494, GY, 38, 3, '#2b2620');
...
export function buildArchive() {
  ...
  lights.push({ x: 620, y: 160, r: 120, color: '#7d6a52', i: 0.24, falloff: 1.4 });

  // Atraves do arquivo morto se chega ao deposito de evidencias. A gaveta
  // vazia deixa de entregar a resposta e passa a apontar para a busca.
  const dEv = M.doorFrame(g, 650, GY, 6345);
  inter.push({
    x: dEv.x, y: dEv.y, w: dEv.w, h: dEv.h, prompt: 'prompt_open',
    action: 'goto', to: 'ch2_evidence', tox: 70, tofacing: 1, range: 30, isDoor: true,
  });
...
// ---------------------------------------------------------------------------
// 2c - ENFERMARIA DO TURNO
//
// Um ramo curto a partir do vestiario. O rele MAO esta preso onde um
// socorrista deixaria as proprias maos: na caixa de primeiros socorros.
// ---------------------------------------------------------------------------

export function buildInfirmary() {
  const W = 720;
  const main = makeBuffer(W, VH);
  const g = main.x;

  rect(g, 0, 0, W, VH, '#161a19');
  ditherV(g, 0, 26, W, 88, '#536058', '#303934', 5);
  for (let x = 0; x < W; x += 16) rect(g, x, 26, 1, 88, '#28312d');
  for (let y = 26; y < 114; y += 16) rect(g, 0, y, W, 1, '#28312d');
  rect(g, 0, 113, W, 3, '#6a736b');
  M.brickWall(g, 0, 116, W, GY - 116, 6401, {
    base: '#343631', hi: '#42443e', dk: '#262824', mortar: '#181a18', moss: false,
  });
  M.asphalt(g, 0, GY, W, VH - GY, 6402, { hi: '#403e38', mid: '#302e29', dk: '#211f1b' });

  const inter = [], lights = [];
  const ex = M.doorFrame(g, 40, GY, 6411);
  inter.push({
    x: ex.x, y: ex.y, w: ex.w, h: ex.h, prompt: 'prompt_open',
    action: 'goto', to: 'ch2_locker', tox: 500, tofacing: -1, range: 30, isDoor: true,
  });

  // Duas macas. Uma delas ainda tem a correia fechada no formato de um
  // corpo que ja nao esta ali.
  for (const bx of [170, 350]) {
    rect(g, bx, GY - 42, 126, 8, '#9a9c90');
    rect(g, bx + 4, GY - 48, 36, 7, '#b7b6a9');
    rect(g, bx + 5, GY - 34, 4, 34, '#5d6464');
    rect(g, bx + 115, GY - 34, 4, 34, '#5d6464');
    rect(g, bx + 14, GY - 2, 10, 3, '#1a1d1d');
    rect(g, bx + 100, GY - 2, 10, 3, '#1a1d1d');
  }
  rect(g, 382, GY - 46, 5, 26, '#5b241e');
  inter.push({ x: 340, y: GY - 56, w: 142, h: 56, prompt: 'prompt_look', lines: 'c2_infirmary_bed', range: 34 });

  // Pia esmaltada e manchas antigas de dedos ao redor do ralo.
  rect(g, 110, 146, 42, 8, '#aaa99e');
  rect(g, 114, 154, 34, 5, '#6f716c');
  rect(g, 128, 137, 4, 10, '#9b9d96');
  rect(g, 119, 159, 4, 35, '#656965');
  rect(g, 141, 159, 4, 35, '#656965');
  inter.push({ x: 106, y: 134, w: 50, h: 62, prompt: 'prompt_look', lines: 'c2_infirmary_sink', range: 28 });

  // Armario de primeiros socorros aberto. O encaixe vazio combina com o
  // rele de latao; a palavra MAO foi riscada por baixo.
  rect(g, 540, 92, 86, 76, '#d1d2c8');
  rect(g, 544, 96, 78, 68, '#272d2b');
  rect(g, 549, 103, 68, 3, '#777d77');
  rect(g, 549, 136, 68, 3, '#777d77');
  rect(g, 572, 110, 18, 18, '#e2e0d5');
  rect(g, 579, 113, 4, 12, '#8c2820'); rect(g, 575, 117, 12, 4, '#8c2820');
  inter.push({
    x: 554, y: 138, w: 42, h: 28, prompt: 'prompt_take',
    action: 'take_relay', relay: 'hand', id: 'relay_hand', range: 28, prio: 2,
  });

  emergencyLamp(g, 340, 46);
  lights.push({ x: 347, y: 56, r: 190, color: '#c7d8c2', i: 0.6, falloff: 1.1 });
  lights.push({ x: 347, y: 190, r: 125, color: '#84947f', i: 0.23, falloff: 1.35 });
  lights.push({ x: 580, y: 126, r: 100, color: '#b7c7af', i: 0.3, flick: 'bulb' });

  const lvl = new Level({
    key: 'ch2_infirmary', nameKey: 'loc_infirmary', width: W, groundY: GY,
    ambient: '#344039', layers: [{ c: main.c, par: 1 }], lightDefs: lights,
    interactables: inter, weather: 'none', reflect: 0.11,
    minX: 30, maxX: W - 40, spawn: { x: 72, facing: 1 }, bloom: 0.45,
    indoor: true, safe: true, ambience: [{ n: 'roomtone', g: 0.1 }],
    randomSfx: [{ fn: 'drip', min: 7, max: 16, vol: 0.65 }],
    maxInimigos: 0, enterBarks: ['b2_inf_1'],
  });
  itensSoltos(lvl, [{
    id: 'relay_hand', x: 568, y: 145,
    draw: (ctx, x, y) => drawRelay(ctx, x, y, 'hand'),
  }]);
  return lvl;
}

// ---------------------------------------------------------------------------
// 2d - SALA DE EVIDENCIAS
//
// O caminho principal continua para dentro do arquivo. Fotografias sem
// rosto e objetos ensacados fazem o rele OLHO parecer parte de um caso.
// ---------------------------------------------------------------------------
...
export function buildEvidence() {
  ...
  const main = makeBuffer(W, VH);
  const g = main.x;
  warehouseWall(g, W, 6501, { corte: 106 });
  damp(g, [214, 610], 108);
  const inter = [], lights = [];

  const ex = M.doorFrame(g, 40, GY, 6511);
  inter.push({
    x: ex.x, y: ex.y, w: ex.w, h: ex.h, prompt: 'prompt_open',
    action: 'goto', to: 'ch2_arquivo', tox: 650, tofacing: -1, range: 30, isDoor: true,
  });

  // Prateleiras de sacos numerados.
  for (const rx of [130, 560]) {
    rack(g, rx, 70, 112, GY - 70, 6520 + rx, true);
  }
  inter.push({ x: 126, y: 70, w: 120, h: 144, prompt: 'prompt_look', lines: 'c2_evidence_bags', range: 34 });

  // Mesa de revelacao: tres fotografias, o mesmo corpo e um rosto raspado.
  rect(g, 286, GY - 46, 206, 7, '#69645b');
  rect(g, 294, GY - 39, 5, 39, '#3b3e40');
  rect(g, 478, GY - 39, 5, 39, '#3b3e40');
  for (let i = 0; i < 3; i++) {
    const px = 308 + i * 55;
    rect(g, px, GY - 73, 42, 30, '#c6c1b5');
    rect(g, px + 3, GY - 70, 36, 23, '#383b3c');
    rect(g, px + 15, GY - 67, 10, 9, '#77766f');
    rect(g, px + 9, GY - 56, 22, 7, '#5b5a55');
    rect(g, px + 14, GY - 66, 13, 3, '#151515');
  }
  inter.push({ x: 294, y: GY - 78, w: 192, h: 40, prompt: 'prompt_look', lines: 'c2_evidence_photos', range: 38 });

  // Caixa de luz com o rele OLHO no encaixe central.
  rect(g, 350, 48, 92, 54, '#76796f');
  rect(g, 355, 53, 82, 44, '#d2cfb8');
  for (let i = 0; i < 6; i++) rect(g, 360 + i * 12, 58, 8, 32, i === 2 ? '#242523' : '#686962');
  inter.push({
    x: 382, y: 62, w: 32, h: 32, prompt: 'prompt_take',
    action: 'take_relay', relay: 'eye', id: 'relay_eye', range: 30, prio: 2,
  });

  const dCom = M.doorFrame(g, 730, GY, 6541);
  inter.push({
    x: dCom.x, y: dCom.y, w: dCom.w, h: dCom.h, prompt: 'prompt_open',
    action: 'goto', to: 'ch2_comms', tox: 70, tofacing: 1, range: 30, isDoor: true,
  });

  emergencyLamp(g, 395, 34);
  lights.push({ x: 402, y: 44, r: 190, color: '#c8bca3', i: 0.58, falloff: 1.12 });
  lights.push({ x: 396, y: 76, r: 105, color: '#ded7b7', i: 0.42, flick: 'bulb' });
  lights.push({ x: 120, y: 182, r: 110, color: '#796c58', i: 0.24 });
  lights.push({ x: 680, y: 182, r: 110, color: '#796c58', i: 0.24 });

  const lvl = new Level({
    key: 'ch2_evidence', nameKey: 'loc_evidence', width: W, groundY: GY,
    ambient: '#302e32', layers: [{ c: main.c, par: 1 }], lightDefs: lights,
    interactables: inter, weather: 'none', reflect: 0.06,
    minX: 30, maxX: W - 40, spawn: { x: 70, facing: 1 }, bloom: 0.44,
    indoor: true, ambience: [{ n: 'hall', g: 0.07 }],
    randomSfx: [{ fn: 'metalCreak', min: 11, max: 25, vol: 0.65 }],
    spawnTipos: ['semrosto'], ritmo: 52, maxInimigos: 1,
    esconderijos: [190, 610], enterBarks: ['b2_ev_1'],
  });
  itensSoltos(lvl, [{
    id: 'relay_eye', x: 391, y: 70,
    draw: (ctx, x, y) => drawRelay(ctx, x, y, 'eye'),
  }]);
  return lvl;
}
...
// 2e - COMUNICACOES
//
// Cabos cortados, gravadores sem fita e o ultimo rele: VOZ. A sala conduz
// diretamente ao posto de seguranca e ao quadro do puzzle.
// ---------------------------------------------------------------------------

export function buildCommunications() {
  const W = 720;
  const main = makeBuffer(W, VH);
  const g = main.x;
  warehouseWall(g, W, 6601, { corte: 96 });
  pipeRun(g, 96, 38, 540, 6602);
  const inter = [], lights = [];

  const ex = M.doorFrame(g, 40, GY, 6611);
  inter.push({
    x: ex.x, y: ex.y, w: ex.w, h: ex.h, prompt: 'prompt_open',
    action: 'goto', to: 'ch2_evidence', tox: 730, tofacing: -1, range: 30, isDoor: true,
  });

  switchboard(g, 150, GY);
  switchboard(g, 222, GY);
  switchboard(g, 294, GY);
  inter.push({ x: 146, y: GY - 52, w: 202, h: 52, prompt: 'prompt_use', lines: 'c2_comms_board', range: 40 });

  // Gravador de rolo. A fita esta ausente; o rele ocupa o cabecote.
  rect(g, 400, GY - 62, 132, 62, '#34383b');
  rect(g, 404, GY - 58, 124, 38, '#16191b');
  for (const cx of [430, 490]) {
    rect(g, cx - 14, GY - 53, 28, 28, '#737773');
    rect(g, cx - 8, GY - 47, 16, 16, '#252928');
    rect(g, cx - 2, GY - 41, 4, 4, '#737773');
  }
  rect(g, 414, GY - 15, 102, 7, '#232728');
  inter.push({ x: 464, y: GY - 22, w: 32, h: 22, prompt: 'prompt_take', action: 'take_relay', relay: 'voice', id: 'relay_voice', range: 28, prio: 2 });
  inter.push({ x: 396, y: GY - 66, w: 142, h: 66, prompt: 'prompt_look', lines: 'c2_comms_tape', range: 36 });

  // Fones pendurados, todos sem cabo.
  for (const hx of [372, 556, 590]) {
    rect(g, hx, 78, 2, 42, '#474b4c');
    rect(g, hx - 5, 118, 12, 4, '#252829');
  }

  const dSec = M.doorFrame(g, 650, GY, 6631);
  inter.push({
    x: dSec.x, y: dSec.y, w: dSec.w, h: dSec.h, prompt: 'prompt_open',
    action: 'goto', to: 'ch2_security', tox: 70, tofacing: 1, range: 30, isDoor: true,
  });

  emergencyLamp(g, 360, 50);
  lights.push({ x: 367, y: 60, r: 194, color: '#b99a70', i: 0.58, falloff: 1.1 });
  lights.push({ x: 470, y: 180, r: 120, color: '#91744e', i: 0.28, flick: 'bulb' });

  const lvl = new Level({
    key: 'ch2_comms', nameKey: 'loc_comms', width: W, groundY: GY,
    ambient: '#322f2b', layers: [{ c: main.c, par: 1 }], lightDefs: lights,
    interactables: inter, weather: 'none', reflect: 0.05,
    minX: 30, maxX: W - 40, spawn: { x: 70, facing: 1 }, bloom: 0.42,
    indoor: true, ambience: [{ n: 'roomtone', g: 0.08 }],
    randomSfx: [{ fn: 'phoneRing', min: 18, max: 36, vol: 0.22 }],
    spawnTipos: ['empilhado'], ritmo: 58, maxInimigos: 1,
    esconderijos: [250], enterBarks: ['b2_com_1'],
  });
  itensSoltos(lvl, [{
    id: 'relay_voice', x: 473, y: GY - 18,
    draw: (ctx, x, y) => drawRelay(ctx, x, y, 'voice'),
  }]);
  return lvl;
}

// ---------------------------------------------------------------------------
// 2f - POSTO DE SEGURANCA
//
// Sala final da investigacao. O painel usa o relogio parado como fonte e
// abre o armario magnetico da chave depois de MAO -> OLHO -> VOZ.
// ---------------------------------------------------------------------------
...
export function buildSecurity() {
  ...
  const main = makeBuffer(W, VH);
  const g = main.x;
  warehouseWall(g, W, 6701, { corte: 104 });
  const inter = [], lights = [];

  const ex = M.doorFrame(g, 40, GY, 6711);
  inter.push({
    x: ex.x, y: ex.y, w: ex.w, h: ex.h, prompt: 'prompt_open',
    action: 'goto', to: 'ch2_comms', tox: 650, tofacing: -1, range: 30, isDoor: true,
  });

  // Janela interna de observacao. Do outro lado, o corredor; no vidro, o
  // reflexo nao acompanha exatamente a sala.
  rect(g, 112, 48, 108, 66, '#171b1d');
  rect(g, 117, 53, 98, 56, '#31404b');
  for (let i = 0; i < 6; i++) rect(g, 120 + i * 17, 54, 2, 54, '#222a30');
  inter.push({ x: 108, y: 44, w: 116, h: 74, prompt: 'prompt_look', lines: 'c2_security_window', range: 32 });

  // Quadro aberto. As vinte placas do minigame correspondem aos quadrados
  // desenhados aqui; a interface apenas aproxima o rosto do personagem.
  rect(g, 268, 52, 174, 126, '#2a3034');
  rect(g, 274, 58, 162, 114, '#101416');
  for (let r = 0; r < 4; r++) for (let c = 0; c < 5; c++) {
    rect(g, 284 + c * 28, 68 + r * 23, 21, 17, '#3c4243');
    rect(g, 286 + c * 28, 70 + r * 23, 17, 2, '#5a6060');
  }
  punchClock(g, 235, 75);
  rect(g, 300, 43, 108, 6, '#4b4030');
  inter.push({
    x: 260, y: 46, w: 190, h: 136, prompt: 'prompt_use',
    action: 'puzzle_panel', range: 48, prio: 2,
  });

  // Armario magnetico. A chave e desenhada apenas depois de o painel
  // resolver, e continua sujeita ao mesmo estado de mundo dos outros itens.
  rect(g, 492, 72, 94, 106, '#3d454b');
  rect(g, 498, 78, 82, 94, '#171c1f');
  rect(g, 502, 82, 74, 42, '#2b3236');
  rect(g, 502, 128, 74, 40, '#252c30');
  rect(g, 565, 112, 7, 12, '#a33a2c');
  inter.push({
    x: 518, y: 104, w: 38, h: 26, prompt: 'prompt_take',
    action: 'take_key', range: 28, id: 'chave', prio: 3, disabled: true,
  });

  const dCor = M.doorFrame(g, 650, GY, 6731);
  inter.push({
    x: dCor.x, y: dCor.y, w: dCor.w, h: dCor.h, prompt: 'prompt_open',
    action: 'goto', to: 'ch2_corridor', tox: 770, tofacing: -1, range: 30, isDoor: true,
  });

  emergencyLamp(g, 474, 34);
  lights.push({ x: 481, y: 44, r: 205, color: '#c2935f', i: 0.62, falloff: 1.1 });
  lights.push({ x: 355, y: 116, r: 118, color: '#b08f3f', i: 0.32, flick: 'bulb' });
  lights.push({ x: 538, y: 122, r: 100, color: '#9d3226', i: 0.22 });

  const lvl = new Level({
    key: 'ch2_security', nameKey: 'loc_security', width: W, groundY: GY,
    ambient: '#302d29', layers: [{ c: main.c, par: 1 }], lightDefs: lights,
    interactables: inter, weather: 'none', reflect: 0.07,
    minX: 30, maxX: W - 40, spawn: { x: 70, facing: 1 }, bloom: 0.48,
    indoor: true, safe: true, ambience: [{ n: 'roomtone', g: 0.08 }],
    randomSfx: [{ fn: 'metalCreak', min: 14, max: 31, vol: 0.45 }],
    maxInimigos: 0, enterBarks: ['b2_sec_1'],
  });
  lvl.keyVisible = false;
  itensSoltos(lvl, [{
    id: 'chave', x: 529, y: 111, visible: () => lvl.keyVisible,
    draw: (ctx, x, y) => {
      ctx.fillStyle = '#d3b75f'; ctx.fillRect(x, y + 2, 12, 2);
      ctx.fillRect(x + 10, y, 5, 6);
      ctx.fillStyle = '#7f622d'; ctx.fillRect(x, y + 4, 3, 2); ctx.fillRect(x + 5, y + 4, 2, 2);
    },
  }]);
  return lvl;
}
...
export function buildChapter2() {
  return {
    ...
    ch2_evidence: buildEvidence(),
    ch2_comms: buildCommunications(),
    ch2_security: buildSecurity(),
    ch2_shelves: buildShelves(),
    ch2_locker: buildLockerRoom(),
    ch2_infirmary: buildInfirmary(),
