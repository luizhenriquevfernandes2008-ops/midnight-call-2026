...
const BOX_X = 28, BOX_W = VW - 56, BOX_H = 58;
const BOX_Y = VH - BOX_H - 16;
const CPS = 44;

// Aproveita o Set que veio de fora em vez de copiar, para que quem chamou
// enxergue o que foi perguntado durante a conversa.
//
// Nao usa `instanceof Set` de proposito: o `Set` da pagina de teste e o `Set`
// do iframe do jogo sao construtores diferentes, e `instanceof` entre os dois
// da falso. O resultado era uma copia silenciosa — a conversa funcionava e a
// memoria de quem chamou ficava vazia. Vale para qualquer codigo que receba
// objeto de outra janela.
function asSet(x) {
  if (x && typeof x.add === 'function' && typeof x.has === 'function') return x;
  return new Set(x || []);
}
...
export class Dialogue {
  ...
  // `opts.memory`  Set de nos ja perguntados que SOBREVIVE a conversa (e ao
  //                save). Sem isso, sair e voltar apagava tudo e a conversa
  //                recomecava do zero — o que empurrou o jogo a proibir
  //                falar duas vezes com o mesmo NPC.
  // `opts.has(f)`  responde se uma flag existe. Pergunta com `req` so
  //                aparece na lista quando a flag correspondente existe, e e
  //                assim que uma deducao do caderno vira fala.
  // `opts.onPress` avisa que ele insistiu num assunto ja respondido.
  startTalk(talk, opts = {}) {
    this.talk = talk;
    this.seen = asSet(opts.memory);
    this.pressed = asSet(opts.pressed);
    this.has = typeof opts.has === 'function' ? opts.has : () => false;
    this.onPress = opts.onPress || null;
    // Avisa que a conversa chegou num no especifico. E assim que a cena do
    // cigarro na cela acontece: ela nao e um botao nem uma escolha, e uma
    // fala que o jogo escuta.
  ...
  // Uma pergunta so entra na lista se a flag exigida existir. Uma pergunta
  // ja feita continua na lista (apagada) — e se o no dela tiver `press`, ela
  // continua clicavel, agora como insistencia.
  _visiveis(list) {
    return list.filter(c => !c.req || this.has(c.req));
  }

  _pressivel(c) {
    if (!c || !c.to || !this.seen.has(c.to)) return false;
    const n = this.talk.nodes[c.to];
    return !!(n && n.press && this.talk.nodes[n.press] && !this.pressed.has(c.to));
  }
  ...
  _node(id) {
    ...
    if (volta) {
      ...
      if (alvo && alvo.choices) {
        const list = this._visiveis(alvo.choices);
        if (list.length) this.pendingChoices = { from: volta, list };
      }
    }
    this._wrapCur();
    if (this.onNode) this.onNode(id, this.talk);
  }

  _pick(c) {
    if (!c) return;
    if (!c.to) { this._endTalk(); return; }
    // Insistir: o assunto ja foi respondido, mas o no tem uma segunda
    // resposta guardada. Custa, e so da para fazer uma vez por assunto.
    if (this._pressivel(c)) {
      const n = this.talk.nodes[c.to];
      this.pressed.add(c.to);
      audio.uiConfirm();
      if (this.onPress) this.onPress(c.to);
      this._node(n.press);
      return;
    }
    this.seen.add(c.to);
  ...
  _wrapCur() {
    ...
    this.wrapped = wrap(l.text, BOX_W - 28, { size: 9, font: 'ui', weight: 'normal' });
  ...
  draw(ctx) {
    ...
    if (l.name) {
      text(ctx, l.name, BOX_X + 12, y - 12, {
        size: 9, font: 'ui', weight: 'bold', color: PAL.uiAccent, track: 0,
        shadow: true, alpha: a,
      });
    }
    ...
    for (const ln of this.wrapped) {
      ...
      text(ctx, part, BOX_X + 14, ty, {
        size: 9, font: 'ui', weight: 'normal',
        color: l.name ? '#f1eadc' : '#e3dccf',
        shadow: true, shadowColor: '#000', shadowOffset: 1, alpha: a,
      });
      shown -= ln.length;
      ty += 12;
    }
  ...
  _drawChoices(ctx, a, boxY) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const c = ult ? null : list[i];
      const feito = !!c && this.seen && this.seen.has(c.to);
      const insistir = !!c && this._pressivel(c);
      // Assunto gasto fica apagado. Assunto ja respondido que AINDA tem uma
      // segunda resposta guardada nao fica apagado — fica marcado, porque o
      // jogador precisa saber que ali ainda tem chao.
      const label = ult ? T('talk_leave')
        : (insistir ? `${tx(c)}  ${T('talk_press')}` : tx(c));
      const cor = sel ? PAL.uiAccent
        : (insistir ? PAL.uiDim : (feito ? PAL.uiFaint : PAL.uiText));
      if (sel) {
      ...
      text(ctx, label, x + 18, y + 5 + i * lh, {
        size: 9, font: 'ui', weight: (feito && !insistir) ? 'normal' : 'bold',
        color: cor, alpha: a, track: 0,
      });
    }
  }
}
...
// ---------------------------------------------------------------------------
// Ainda ha assunto com esse NPC?
//
// Antes, falar duas vezes com alguem reiniciava a conversa inteira, entao o
// jogo passou a proibir isso (sessao 12). Com memoria persistente da para
// permitir de novo — mas so quando existe pergunta nova ou uma insistencia
// disponivel. Conversa esgotada continua sem reabrir, que era o ponto da
// correcao original.
// ---------------------------------------------------------------------------

export function talkHasMore(talk, seen, pressed, has) {
  if (!talk) return false;
  const S = asSet(seen);
  const P = asSet(pressed);
  const H = typeof has === 'function' ? has : () => false;
  for (const id in talk.nodes) {
    const n = talk.nodes[id];
    if (!n.choices) continue;
    for (const c of n.choices) {
      if (c.req && !H(c.req)) continue;
      if (!c.to) continue;
      if (!S.has(c.to)) return true;
      const alvo = talk.nodes[c.to];
      if (alvo && alvo.press && talk.nodes[alvo.press] && !P.has(c.to)) return true;
    }
  }
  return false;
}
