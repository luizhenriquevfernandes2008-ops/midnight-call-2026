...
export const TIPOS = {
  ...
  empilhado: {
    anim: 'crawl', vel: 34, hp: 12, dano: 15, alcance: 24, recarga: 1.28,
  ...
  semrosto: {
    ...
    // de nenhuma.
    anim: 'shamble', vel: 27, hp: 10, dano: 11, alcance: 21, recarga: 1.52,
...
export class Enemy {
  constructor(tipo, x, y) {
    ...
    this.vivo = true;
    this._px = x;
    this.wounds = { head: 0, torso: 0, arms: 0, legs: 0, feet: 0 };
    this.aggression = 0;
    this.frenzyContact = false;
    this.frenzyChecked = false;
    this.alertT = 0;
    this.diff = { enemySpeed: 1, enemyDamage: 1, enemyHp: 1, frenzyChance: 0.3 };
    ...
    // como uma ideia.
    // Copia rasa: um headshot pode remover a cabeca DESTE inimigo sem
    // apagar a peca compartilhada dos proximos que nascerem.
  ...
  setDifficulty(diff) {
    if (!diff) return;
    const oldMax = this.maxHp || this.cfg.hp || 1;
    const ratio = clamp(this.hp / oldMax, 0, 1);
    this.diff = diff;
    if (!this.cfg.invulneravel && this._difficultyId !== diff.id) {
      const first = !this._difficultyApplied;
      this._difficultyApplied = true; this._difficultyId = diff.id;
      this.maxHp = Math.max(1, Math.round(this.cfg.hp * (diff.enemyHp || 1)));
      this.hp = first ? this.maxHp : Math.max(1, this.maxHp * ratio);
    }
  }
  ...
  _evento(ev) {
    ...
    if (ev === 'crawlstep') audio.step(false, 0.34 * v, 'concrete');
    else if (ev === 'softstep') audio.step(false, 0.24 * v, 'concrete');
    else if (ev === 'dragstep') audio.step(false, 0.32 * v, 'metal');
  ...
  update(dt, player, level) {
    ...
    // O piso entrega David. Metal e corrida geram um surto curto de
    // direcao e velocidade, sem conceder visao magica atraves da sala.
    if (player.noiseT > 0 && Math.abs((player.noiseX || player.x) - this.x) <= (player.noiseRange || 0)) {
      this.alertT = Math.max(this.alertT, player.noiseT + 0.75);
    }
    ...
    if (this.state === 'frenzy') {
      ...
      const ferocidade = 1.95 + this.aggression * 0.75;
      const v = this.cfg.vel * ferocidade * (this.diff.enemySpeed || 1);
      if (dist > 15) this.x += dir * v * dt;
      if (level) this.x = clamp(this.x, level.minX - 20, level.maxX + 20);
      d.speed = clamp(v / 78, 1.1, 2.2);
      if (d.anim !== 'frenzyRun') d.play('frenzyRun', { blend: 0.06 });
      d.update(dt);
      if (dist <= 19) this.frenzyContact = true;
      return;
    }
    ...
    if (dist <= this.cfg.alcance && this.cd <= 0) {
      this.state = 'attack';
      this.cd = this.cfg.recarga / (1 + this.aggression * 0.42);
      this.facing = dir; d.setFacing(dir);
    ...
    if (dist > parada) {
      this.facing = dir;
      d.setFacing(dir);
      const perna = this.wounds.legs + this.wounds.feet;
      const ferida = perna >= 3 ? 0.58 : perna >= 1 ? 0.82 : 1;
      const alerta = this.alertT > 0 ? 1.26 : 1;
      const furia = 1 + this.aggression * 0.38;
      const v = this.cfg.vel * ferida * alerta * furia * (this.diff.enemySpeed || 1);
      this.x += dir * v * dt;
      if (level) this.x = clamp(this.x, level.minX - 30, level.maxX + 30);
      const anim = this.tipo === 'empilhado'
        ? (perna >= 2 || this.wounds.arms >= 2 ? 'woundedCrawl' : this.cfg.anim)
        : (perna >= 2 ? 'limp' : this.cfg.anim);
      d.speed = clamp(v / Math.max(1, this.cfg.vel), 0.65, 1.75);
      if (d.anim !== anim) d.play(anim, { blend: 0.16 });
      if (this.cfg.encara && this.aggression < 0.28 && Math.random() < dt * 0.018) this.encaraT = 0.55 + Math.random() * 0.7;
  ...
  levarDano(n, deX, zona = 'torso') {
    if (this.state === 'dead') return false;
    this.det.blood = Math.min(1, (this.det.blood || 0) + (zona === 'head' ? 0.55 : 0.22));
    if (this.wounds[zona] !== undefined) this.wounds[zona]++;
    ...
    this.hp -= n;
    this.aggression = clamp(1 - this.hp / Math.max(1, this.maxHp), 0, 1);
    this._aplicarFerida(zona);
    ...
    if (this.hp / this.maxHp <= 0.38 && !this.frenzyChecked) {
      this.frenzyChecked = true;
      const chance = this.forceFrenzy ? 1 : (this.diff.frenzyChance || 0.3);
      if (Math.random() < chance) {
        this.state = 'frenzy';
        this.stun = 0;
        this.det.play('frenzyRun', { restart: true, blend: 0.035 });
        audio.strain(0.9);
        return 'frenzy';
      }
    }
  ...
  _aplicarFerida(zona) {
    const p = { ...(this.det.parts || {}) };
    if (zona === 'arms' && this.wounds.arms >= 2) {
      p.forearm = null; p.hand = null;
    }
    if ((zona === 'legs' || zona === 'feet') && this.wounds.legs + this.wounds.feet >= 3) {
      p.foot = null;
    }
    this.det.parts = p;
  }
  ...
  finalizar() {
    if (this.state === 'dead') return false;
    this.hp = 0; this.state = 'dead'; this.morto = 0.72;
    this.det.blood = 1;
    return true;
  }

  explodirCabeca() {
    if (this.headGone || this.cfg.invulneravel || this.fake) return false;
    this.headGone = true;
    this.det.parts = { ...(this.det.parts || {}), head: null, headBlank: null };
    this.det.blood = 1;
    return true;
  }
...
export class Director {
  constructor() {
    this.lista = [];
    this.calma = 7;           // segundos de silencio ainda devidos
    this.ultimoX = -9999;
    this.ligado = false;
    this.onSpawn = null;
  ...
  update(dt, ctx) {
    ...
    const extra = this.difficulty ? (this.difficulty.maxEnemies || 0) : 0;
    const limite = teto <= 0 ? 0 : Math.min(TETO + 2, Math.max(1, teto + extra));
    if (this.vivos >= limite) return;
    ...
    const ritmoDiff = this.difficulty ? (this.difficulty.spawnRate || 1) : 1;
    const base = (level.ritmo || 26) * 0.42 / pressa / ritmoDiff;
    ...
    if (s < 50 && tipo !== 'ecoador' && Math.random() < (s < 25 ? 0.45 : 0.22)) {
      e.fake = true;
      e.alpha = 0.72; e.det.alpha = 0.72; e.det.rimAlpha = 0.12; e.det.reflect = 0;
    }
    this.lista.push(e);
    this.ultimoX = x;
    this.calma = 1.0;
  ...
  // A bala atravessa toda a reta e devolve todos os corpos em ordem. Alem
  // da altura, informa a regiao atingida para dano anatomico.
  naLinhaDoTiroTodos(ox, oy, facing, angGraus, alcance = 240) {
    ...
    for (const e of this.lista) {
      ...
      const rel = clamp((y - b.y0) / Math.max(1, b.y1 - b.y0), 0, 1);
      let zona = 'torso', dano = 2.4;
      if (rel < 0.22) { zona = 'head'; dano = 5; }
      else if (rel < 0.48) { zona = 'torso'; dano = 2.4; }
      else if (rel < 0.68) { zona = 'arms'; dano = 1.6; }
      else if (rel < 0.90) { zona = 'legs'; dano = 1.35; }
      else { zona = 'feet'; dano = 1; }
      hits.push({ alvo: e, d: dx, y, zona, dano });
    }
  ...
  naLinhaDoTiro(ox, oy, facing, angGraus, alcance = 240) {
    const h = this.naLinhaDoTiroTodos(ox, oy, facing, angGraus, alcance);
    return h.length ? h[0].alvo : null;
  }
