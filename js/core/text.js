...
const FONTS = {
  ...
  // escrevendo.
  type:   '"Courier New",Courier,Consolas,"DejaVu Sans Mono",monospace',
};
...
function key(str, o) {
  return `${o.font}|${o.size}|${o.weight}|${o.color}|${o.track}|${o.threshold}|${o.smooth}|${str}`;
}
...
function bake(str, o) {
  ...
  // Titulos continuam com corte duro. Texto corrido conserva o alpha da
  // fonte do sistema: transformar toda borda em um bloco opaco era o que
  // colava letras pequenas umas nas outras.
  ...
  for (let p = 0; p < d.length; p += 4) {
    if (d[p + 3] >= thr) {
      const alphaOriginal = d[p + 3];
      d[p] = rgb[0]; d[p + 1] = rgb[1]; d[p + 2] = rgb[2];
      d[p + 3] = o.smooth ? alphaOriginal : 255;
      const idx = p >> 2;
...
function opts(o = {}) {
  return {
    ...
    smooth: o.smooth === undefined ? o.font !== 'title' : !!o.smooth,
    threshold: o.threshold === undefined ? (o.font === 'title' ? 0.48 : 0.03) : o.threshold,
  };
}
